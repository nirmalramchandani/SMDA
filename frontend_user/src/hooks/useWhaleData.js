/**
 * WhaleTrace — Data Hook
 * Fetches and caches all whale data from the SMDA backend
 */
  ...
  useEffect(() => {
    ...
    if (!fetchPromise) {
      fetchPromise = Promise.all([
        fetch(`${API_BASE}/data/investors?limit=5000`),
        fetch(`${API_BASE}/data/sells?limit=5000`),
        fetch(`${API_BASE}/data/transactions?limit=2000`),
      ])
        .then(async ([invResp, sellResp, txResp]) => {
          if (!invResp.ok || !sellResp.ok || !txResp.ok)
            throw new Error('Failed to connect to WhaleTrace backend.');
