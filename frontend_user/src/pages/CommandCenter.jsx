  ...
  if (loading) {
    return (
      <div className="loader-container">
        ...
        <p className="loader-text">Initializing WhaleTrace Terminal...</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6 animate-in">
        <div>
          ...
          <p className="page-subtitle">
            Institutional smart money flow across {investors.length.toLocaleString()} tracked entities
          </p>
        </div>
      ...
      <div className="grid-4 mb-6">
        <div className="stat-card animate-in animate-in-delay-1">
          <div className="stat-label"><Users size={14} /> Tracked Whales</div>
          <div className="stat-value text-info">{stats.activeWhales.toLocaleString()}</div>
          <div className="stat-delta text-muted">Unique investor entities</div>
        </div>
        <div className="stat-card animate-in animate-in-delay-2">
          <div className="stat-label"><Activity size={14} /> Period Exits</div>
          <div className="stat-value">{stats.totalDeals.toLocaleString()}</div>
          <div className="stat-delta text-muted">Closed positions in range</div>
        </div>
        <div className="stat-card animate-in animate-in-delay-3">
          <div className="stat-label"><TrendingUp size={14} /> Aggregate PnL</div>
          <div className={`stat-value ${stats.totalPnl >= 0 ? 'text-success' : 'text-danger'}`}>
            {formatINR(stats.totalPnl)}
          </div>
          <div className="stat-delta text-muted">Net realized across all exits</div>
        </div>
        <div className="stat-card animate-in animate-in-delay-4">
          <div className="stat-label"><Target size={14} /> Win Rate</div>
          <div className="stat-value" style={{ color: stats.winRate > 50 ? 'var(--accent-emerald)' : 'var(--accent-amber)' }}>
            {stats.winRate.toFixed(1)}%
          </div>
          <div className="stat-delta text-muted">Profitable exit ratio</div>
        </div>
      </div>
      ...
      <div className="grid-3 mb-6">
        ...
        <div className="glass-card animate-in" style={{ gridColumn: 'span 2' }}>
          <div className="section-title"><Zap size={14} /> Cumulative Smart Money PnL</div>
